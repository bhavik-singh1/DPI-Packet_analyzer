...
The benchmark tool measures and demonstrates the **real-world performance** of the DPI engine by tracking metrics such as processing speed, throughput, scalability, and memory usage.

### Why Benchmark?

| Question | What It Measures |
|----------|-----------------|
| How fast is it? | Packets per second (PPS), throughput (Mbps) |
| Does multithreading help? | Single-thread vs multi-thread speedup ratio |
| How much traffic can it handle? | Peak throughput capacity |
| How much RAM does it consume? | Peak memory (RSS), memory per packet |
| Is the architecture efficient? | Per-thread utilization, scaling efficiency % |

### Building the Benchmark

```bash
g++ -std=c++17 -pthread -O2 -I include -o dpi_benchmark.exe src/benchmark.cpp src/pcap_reader.cpp src/packet_parser.cpp src/sni_extractor.cpp src/types.cpp -lpsapi
```

### Running the Benchmark

**Basic (uses existing test_dpi.pcap, multiplied 100x):**
```bash
./dpi_benchmark.exe test_dpi.pcap
```

**Custom settings:**
```bash
./dpi_benchmark.exe test_dpi.pcap --multiply 500 --lbs 2 --fps 4 --block-app YouTube
```

| Option | Default | Description |
|--------|---------|-------------|
| `--multiply <N>` | 100 | Replicate input packets N times for a larger workload |
| `--lbs <n>` | 2 | Number of Load Balancer threads |
| `--fps <n>` | 2 | Fast Path threads per LB |
| `--block-app <app>` | (none) | Add a blocking rule to test filtering overhead |

### How It Works

```
┌──────────────┐       ┌──────────────────────────────────────┐
│ input.pcap   │──────►│        BENCHMARK RUNNER               │
│ (77 packets) │       │                                      │
└──────────────┘       │  1. Load packets from PCAP           │
                       │  2. Multiply × N (e.g., 500)         │
                       │  3. Measure memory (before)           │
                       │                                      │
                       │  ┌─ RUN 1: Single-Thread ──────────┐ │
                       │  │  1 LB × 1 FP                    │ │
                       │  │  Timer START → process → STOP   │ │
                       │  │  Record PPS, throughput, time    │ │
                       │  └─────────────────────────────────┘ │
                       │                                      │
                       │  ┌─ RUN 2: Multi-Thread ───────────┐ │
                       │  │  N LB × M FP                    │ │
                       │  │  Timer START → process → STOP   │ │
                       │  │  Record PPS, throughput, time    │ │
                       │  │  Record per-thread stats         │ │
                       │  └─────────────────────────────────┘ │
                       │                                      │
                       │  4. Measure memory (after)            │
                       │  5. Calculate speedup ratio           │
                       │  6. Print BENCHMARK REPORT            │
                       └──────────────────────────────────────┘
```
...
```
======================================================================
                    DPI ENGINE BENCHMARK REPORT
======================================================================
  Input File:           test_dpi.pcap
  Packet Multiplier:    500x
  Total Packets:        38,500
  Total Data:           2.7 MB
----------------------------------------------------------------------
                           SPEED METRICS
----------------------------------------------------------------------
                     Single-Thread     Multi-Thread     Speedup
  Time:                    16.4 ms         25.2 ms       0.7x
  Packets/sec:        2,340,994    1,529,434       0.7x
  Throughput:          1395.6 Mbps      911.8 Mbps      0.7x
  Avg Latency:            0.4 us          0.7 us       0.7x
----------------------------------------------------------------------
                        SCALABILITY ANALYSIS
----------------------------------------------------------------------
  Thread Config:        2 LBs x 2 FPs = 4 worker threads
  Speedup Factor:       0.65x (ideal: 4.00x)
  Efficiency:           16.3% of ideal linear scaling
----------------------------------------------------------------------
                      PER-THREAD UTILIZATION
----------------------------------------------------------------------
  LB0 dispatched:         26,500  (68.8%)
  LB1 dispatched:         12,000  (31.2%)
  FP0 processed:          26,500  (68.8%)  #################
  FP1 processed:               0  ( 0.0%)
  FP2 processed:               0  ( 0.0%)
  FP3 processed:          12,000  (31.2%)  #######
----------------------------------------------------------------------
                        RESOURCE USAGE
----------------------------------------------------------------------
  Peak Memory (RSS):     1.1 MB
  Memory per Packet:     30.1 bytes
  Forwarded Packets:     37,002
  Dropped Packets:       1,498
======================================================================
```
...
| Metric | Meaning |
|--------|---------|
| **Packets/sec (PPS)** | Raw processing speed — how many packets the engine can inspect per second |
| **Throughput (Mbps)** | Data throughput — total bits processed per second |
| **Avg Latency (µs)** | Average time to process a single packet |
| **Speedup Factor** | How much faster multi-thread is vs single-thread (>1 = faster) |
| **Efficiency %** | Speedup / number-of-threads × 100. 100% = perfect linear scaling |
| **Per-Thread Utilization** | How evenly work is distributed across Fast Path threads |
| **Peak Memory (RSS)** | Resident Set Size — actual RAM consumed by the process |

> **Note:** With small PCAP files (<1000 packets), single-thread may outperform multi-thread because thread synchronization overhead exceeds the processing work. Use `--multiply 500` or higher for realistic scaling results.

### Generating Large Test Data

For more meaningful benchmarks, generate a large PCAP:

```bash
python generate_benchmark_pcap.py
./dpi_benchmark.exe benchmark_test.pcap --multiply 10 --lbs 4 --fps 4
```

This creates `benchmark_test.pcap` with ~5000+ diverse packets (TLS, HTTP, DNS) from multiple source IPs.

---

## 13. Docker & Containerization

The project includes production-ready Docker support for running the DPI engine without needing to install C++ compilers natively. It uses an optimized multi-stage Alpine Linux build resulting in a minimal footprint.
...
To analyze PCAP files, you must mount a local directory to `/app/pcaps` inside the container. 

1. Create a directory and place your input file in it:
   ```bash
   mkdir pcaps
   cp test_dpi.pcap pcaps/input.pcap
   ```
...
The output file will be written to `./pcaps/output.pcap` on your host machine.

### Using Environment Variables

You can configure the thread counts and blocking rules dynamically using environment variables without passing complex CLI flags:
...
docker run --rm \
  ...
  -e FPS=4 \
  -e BLOCK_APP=YouTube \
  -e BLOCK_IP=192.168.1.50 \
  dpi-engine
```

**Supported Variables:**
- `INPUT_FILE` (default: `/app/pcaps/input.pcap`)
- `OUTPUT_FILE` (default: `/app/pcaps/output.pcap`)
- `LBS`: Number of Load Balancer threads
- `FPS`: Number of Fast Path threads per LB
- `BLOCK_APP`: App to block (e.g., YouTube)
- `BLOCK_IP`: IP to block
- `BLOCK_DOMAIN`: Domain to block

*(Note: You can still bypass the entrypoint entirely and pass raw CLI arguments to `dpi_engine` if you append them to your `docker run` command).*

### Using Docker Compose

For a declarative setup, use `docker-compose`. Edit `docker-compose.yml` to set your desired environment variables and run:

```bash
docker-compose up
```

This will automatically build the image if it doesn't exist, mount the `./pcaps` volume, apply the rules, and execute the engine.
...
6. **Performance Benchmarking** - Measuring speed, scalability, and resource usage
