...
private:
    ...
    void run() {
        while (running_) {
            ...
            // DAY 4 FIX: Lock-Free Consistent Hashing
            // We hash the 5-tuple so that all packets from the same connection
            // will always be routed to the exact same Fast Path thread.
            // This guarantees thread-safety for our flow maps without needing mutexes!
